...
WEAPON_FACTORY_SLOTS_PER_CITY = 2
WEAPON_FACTORY_BASE_PRICE = 750000
WEAPON_FACTORY_PRICE_MULTIPLIER = 1.6
BULLET_UNIT_PRICE = 20
WEAPON_FACTORY_BULLET_PRODUCTION_PER_HOUR = 250
WEAPON_FACTORY_STOCK_CAPS = {
    "bullet_stock": 10000,
    "razor_stock": 100,
    "revolver_stock": 60,
    "shotgun_stock": 35,
    "tommy_gun_stock": 15,
    "sniper_rifle_stock": 8,
}

PRODUCTION_FACTORY_TYPES = {
    "whiskey_distillery": {"name": "Whiskey Distillery", "cost": 15000, "kind": "warehouse", "item_key": "whiskey", "interval": 60, "amount": 1, "unit_label": "Whiskey"},
    "rum_distillery": {"name": "Rum Distillery", "cost": 18000, "kind": "warehouse", "item_key": "rum", "interval": 60, "amount": 1, "unit_label": "Rum"},
    "cigar_factory": {"name": "Cigar Factory", "cost": 25000, "kind": "warehouse", "item_key": "cigars", "interval": 60, "amount": 1, "unit_label": "Cigars"},
    "luxury_watches_factory": {"name": "Luxury Watches Factory", "cost": 250000, "kind": "warehouse", "item_key": "luxury_watches", "interval": 3600, "amount": 1, "unit_label": "Luxury Watches"},
    "weapon_crates_factory": {"name": "Weapon Crates Factory", "cost": 500000, "kind": "warehouse", "item_key": "weapons", "interval": 3600, "amount": 1, "unit_label": "Weapon Crates"},
    "car_factory": {"name": "Car Factory", "cost": 1500000, "kind": "vehicle", "item_key": None, "interval": 10800, "amount": 1, "unit_label": "Vehicle"},
}
...
CAR_FACTORY_RANK_VEHICLES = [
    ...
    ("Chief Enforcer", "austin_12_heavy"),
    ("Caporegime", "ford_model_t"),
    ("Trusted Lieutenant", "luxury_carriage_pair"),
    ("Crew Leader", "horse_goods_cart"),
    ("Made Man", "raleigh_motor_bicycle"),
    ("Shelby Associate", "canal_work_horse"),
    ("Street Runner", "birmingham_courier_bicycle"),
]

WEAPON_FACTORY_WEAPON_PRODUCTION_PER_HOUR = {
    "razor": 5,
    "revolver": 3,
    "shotgun": 1,
    "tommy_gun": 0.5,
...
CONTRABAND = {
    ...
    "weapons": {"label": "Weapon Crates", "risk": 14, "prices": {"Birmingham": 2200, "London": 2800, "Liverpool": 2400, "Manchester": 2600, "Chicago": 1400, "New York": 1600, "Dublin": 3200, "Glasgow": 3000, "Paris": 4200, "Amsterdam": 3600, "Havana": 1000}},
}
...
def best_smuggling_route(item_key):
    if item_key not in CONTRABAND:
        return None
    rows = []
    for buy_city in CITIES:
        buy_price = int(dynamic_market_price(buy_city, item_key))
        for sell_city in CITIES:
            if sell_city == buy_city:
                continue
            sell_price = int(dynamic_market_price(sell_city, item_key))
...
class User(db.Model):
    ...
    reset_expires_at = db.Column(db.Float, default=0.0)
...
class CityWeaponFactory(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    city = db.Column(db.String(40), nullable=False)
    slot_number = db.Column(db.Integer, nullable=False)
    name = db.Column(db.String(100), nullable=False)
    owner_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=True)
    price = db.Column(db.Integer, default=WEAPON_FACTORY_BASE_PRICE)
    bullet_stock = db.Column(db.Integer, default=1000)
    razor_stock = db.Column(db.Integer, default=10)
    revolver_stock = db.Column(db.Integer, default=5)
    shotgun_stock = db.Column(db.Integer, default=2)
    tommy_gun_stock = db.Column(db.Integer, default=0)
    sniper_rifle_stock = db.Column(db.Integer, default=0)
    last_production = db.Column(db.Float, default=time.time)
    created_at = db.Column(db.Float, default=time.time)

    owner = db.relationship("User", foreign_keys=[owner_id], backref=db.backref("owned_weapon_factories", lazy=True))


class CityFactory(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
    city = db.Column(db.String(40), nullable=False)
    factory_type = db.Column(db.String(80), nullable=False)
    quantity = db.Column(db.Integer, default=0)
    pending_quantity = db.Column(db.Integer, default=0)
    last_production = db.Column(db.Float, default=0.0)

    user = db.relationship("User", backref=db.backref("city_factories", lazy=True))
...
class MarketplaceListing(db.Model):
    ...
    quantity = db.Column(db.Integer, default=0)
    price_each = db.Column(db.Integer, default=0)
...
class MarketplaceTrade(db.Model):
    id = db.Column(db.Integer, primary_key=True)
...
class StateTreasury(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    balance = db.Column(db.Integer, default=0)
    marketplace_tax_collected = db.Column(db.Integer, default=0)
    created_at = db.Column(db.Float, default=time.time)
...
def marketplace_item_label(item_type, item_key):
    if item_type == "bullets":
        return "Bullets"
    if item_type == "weapon":
        return WEAPON_TYPES.get(item_key, {}).get("name", item_key)
    if item_type == "goods":
        return CONTRABAND.get(item_key, {}).get("label", item_key)
    if item_type == "vehicle":
        data = VEHICLE_BY_KEY.get(item_key, {})
        if data:
            return f"{data.get('year', '')} {data.get('name', item_key)}".strip()
        return item_key
    return item_key


def marketplace_active_listings():
    return MarketplaceListing.query.filter(
        MarketplaceListing.is_active == True,
        MarketplaceListing.quantity > 0
    ).order_by(MarketplaceListing.created_at.desc()).limit(200).all()


def marketplace_create_options(user):
    options = {"goods": [], "weapons": [], "vehicles": [], "bullets": int(safe_number(getattr(user, "bullets", 0)))}
    if not user or not user.id:
        return options
    ...
    try:
        ensure_warehouse_items(user)
        for key, data in CONTRABAND.items():
            qty = warehouse_quantity(user, key)
            if qty > 0:
                options["goods"].append({"key": key, "label": data["label"], "quantity": qty})
    except Exception:
        pass
    ...
    try:
        ...
        for key, data in WEAPON_TYPES.items():
            qty = weapon_quantity(user, key)
            if qty > 0:
                options["weapons"].append({"key": key, "label": data["name"], "quantity": qty})
    except Exception:
        pass

    try:
        ensure_city_vehicles(user)
        for item in owned_vehicles_in_city(user, user.location):
            vehicle = item["vehicle"]
            options["vehicles"].append({
                "key": vehicle["key"],
                "label": f"{vehicle['year']} {vehicle['name']}",
...
def marketplace_remove_from_seller(user, item_type, item_key, amount):
    amount = int(amount)
    if amount <= 0:
        return False, "Invalid amount."

    if item_type == "bullets":
        if int(safe_number(user.bullets)) < amount:
            return False, "Not enough bullets."
        user.bullets = int(safe_number(user.bullets)) - amount
        return True, ""

    if item_type == "goods":
        if item_key not in CONTRABAND:
            return False, "Invalid goods."
        row = warehouse_item(user, item_key)
        if int(safe_number(row.quantity)) < amount:
            return False, "Not enough goods."
        row.quantity = int(safe_number(row.quantity)) - amount
        return True, ""

    if item_type == "weapon":
        if item_key not in WEAPON_TYPES:
            return False, "Invalid weapon."
        row = UserWeapon.query.filter_by(user_id=user.id, weapon_key=item_key).first()
        if not row or int(safe_number(row.quantity)) < amount:
            return False, "Not enough weapons."
        row.quantity = int(safe_number(row.quantity)) - amount
        return True, ""

    if item_type == "vehicle":
        if item_key not in VEHICLE_BY_KEY:
            return False, "Invalid vehicle."
        if not remove_vehicle_from_city(user, user.location, item_key, amount):
            return False, f"Not enough vehicles in {user.location}."
        return True, ""

    return False, "Invalid item type."


def marketplace_give_item(user, item_type, item_key, amount, city=None):
    amount = int(amount)
    if amount <= 0:
        return
    ...
    if item_type == "bullets":
        ...
        add_weapon(user, item_key, amount)
    elif item_type == "vehicle":
        add_vehicle_to_city(user, city or user.location, item_key, amount)


def marketplace_listing_fee(total_value):
    return max(1, int(total_value * MARKETPLACE_FEE_RATE))


def state_treasury():
    treasury = StateTreasury.query.first()
    if not treasury:
        treasury = StateTreasury(balance=0, marketplace_tax_collected=0, created_at=time.time())
        db.session.add(treasury)
        db.session.commit()
    return treasury


def add_state_treasury(amount):
    amount = int(amount)
    if amount <= 0:
        return
    treasury = state_treasury()
    treasury.balance = int(safe_number(treasury.balance)) + amount
    treasury.marketplace_tax_collected = int(safe_number(treasury.marketplace_tax_collected)) + amount


def marketplace_filtered_listings(item_type=None, query=None):
    q = MarketplaceListing.query.filter(
        MarketplaceListing.is_active == True,
        MarketplaceListing.quantity > 0
    )

    if item_type and item_type != "all":
        q = q.filter(MarketplaceListing.item_type == item_type)

    listings = q.order_by(MarketplaceListing.created_at.desc()).limit(500).all()

    query = (query or "").strip().lower()
    if query:
        filtered = []
        for listing in listings:
            seller_name = (listing.seller.username if listing.seller else "").lower()
            item_name = marketplace_item_label(listing.item_type, listing.item_key).lower()
            if query in seller_name or query in item_name:
...
def marketplace_stats(listings=None):
    active = marketplace_active_listings()
    treasury = None
    try:
        treasury = state_treasury()
    except Exception:
    ...
    value = sum(int(safe_number(l.quantity)) * int(safe_number(l.price_each)) for l in active)
    since = time.time() - 86400
    trades_today = MarketplaceTrade.query.filter(MarketplaceTrade.created_at >= since).count()

    item_counts = {}
    for trade in MarketplaceTrade.query.order_by(MarketplaceTrade.created_at.desc()).limit(250).all():
        label = marketplace_item_label(trade.item_type, trade.item_key)
        item_counts[label] = item_counts.get(label, 0) + int(safe_number(trade.quantity))

    most_traded = "-"
    if item_counts:
        most_traded = sorted(item_counts.items(), key=lambda row: row[1], reverse=True)[0][0]
    ...
    return {
        ...
        "treasury": int(safe_number(treasury.balance)) if treasury else 0,
    }


def recent_marketplace_trades(limit=25):
    return MarketplaceTrade.query.order_by(MarketplaceTrade.created_at.desc()).limit(limit).all()
...
def weapon_factory_price_for_slot(slot_number):
    return int(WEAPON_FACTORY_BASE_PRICE * (WEAPON_FACTORY_PRICE_MULTIPLIER ** (slot_number - 1)))


def weapon_stock_field(weapon_key):
    return f"{weapon_key}_stock"


def ensure_city_weapon_factories():
    """Create limited weapon factories per city. State-owned factories also produce stock."""
    changed = False
    for city in CITIES:
        for slot in range(1, WEAPON_FACTORY_SLOTS_PER_CITY + 1):
            factory = CityWeaponFactory.query.filter_by(city=city, slot_number=slot).first()
            if not factory:
                factory = CityWeaponFactory(
                    city=city,
                    slot_number=slot,
                    name=f"{city} Weapon Factory #{slot}",
                    owner_id=None,
                    price=weapon_factory_price_for_slot(slot),
                    bullet_stock=1000,
                    razor_stock=10,
                    revolver_stock=5,
                    shotgun_stock=2,
                    tommy_gun_stock=0,
                    sniper_rifle_stock=0,
                    last_production=time.time(),
                )
...
def update_weapon_factory_production(factory):
    if not factory:
        return
    now = time.time()
    if safe_number(getattr(factory, "last_production", 0)) <= 0:
        factory.last_production = now
        return

    elapsed = int(now - safe_number(factory.last_production))
    hours = elapsed // 3600
    if hours <= 0:
        return

    factory.bullet_stock = min(
        WEAPON_FACTORY_STOCK_CAPS["bullet_stock"],
        int(safe_number(factory.bullet_stock)) + (hours * WEAPON_FACTORY_BULLET_PRODUCTION_PER_HOUR),
    )

    for weapon_key, production in WEAPON_FACTORY_WEAPON_PRODUCTION_PER_HOUR.items():
        field = weapon_stock_field(weapon_key)
        cap = int(WEAPON_FACTORY_STOCK_CAPS.get(field, 50))
        current = int(safe_number(getattr(factory, field, 0)))
        # Fractional production allows elite weapons to appear slowly.
        produced = int(hours * float(production))
        if produced > 0:
            setattr(factory, field, min(cap, current + produced))

    factory.last_production = safe_number(factory.last_production) + (hours * 3600)


def update_city_weapon_factories(city):
    ensure_city_weapon_factories()
    factories = CityWeaponFactory.query.filter_by(city=city).order_by(CityWeaponFactory.slot_number.asc()).all()
    for factory in factories:
        update_weapon_factory_production(factory)
    db.session.commit()
    return factories


def city_weapon_factories(city):
    return update_city_weapon_factories(city)


def weapon_factory_owner_label(factory):
    if not factory or not factory.owner_id:
        return "State"
    if not factory.owner or factory.owner.is_dead:
        return "State"
    return factory.owner.username


def total_factory_bullet_stock(city):
    return sum(int(safe_number(f.bullet_stock)) for f in city_weapon_factories(city))


def total_factory_weapon_stock(city, weapon_key):
    field = weapon_stock_field(weapon_key)
    return sum(int(safe_number(getattr(f, field, 0))) for f in city_weapon_factories(city))


def weapon_factory_overview_rows(city):
    factories = city_weapon_factories(city)
    rows = []
    for factory in factories:
        weapon_total = 0
        for weapon_key in WEAPON_TYPES:
            weapon_total += int(safe_number(getattr(factory, weapon_stock_field(weapon_key), 0)))
        rows.append({
            "factory": factory,
            "owner": weapon_factory_owner_label(factory),
            "bullet_stock": int(safe_number(factory.bullet_stock)),
            "weapon_stock": weapon_total,
            "price": int(safe_number(factory.price)),
            "is_state": not bool(factory.owner_id),
        })
    return rows


def consume_factory_stock(user, city, stock_field, amount, unit_price):
    amount = int(amount)
    if amount <= 0:
        return False, "Invalid amount."

    factories = city_weapon_factories(city)
    available = sum(int(safe_number(getattr(factory, stock_field, 0))) for factory in factories)
    if available < amount:
        return False, f"Not enough local factory stock in {city}. Available: {available}."

    total_cost = amount * int(unit_price)
    if safe_number(user.money) < total_cost:
        return False, "Not enough cash."

    user.money = int(safe_number(user.money)) - total_cost
    remaining = amount

    for factory in factories:
        if remaining <= 0:
            break
        current = int(safe_number(getattr(factory, stock_field, 0)))
        if current <= 0:
            continue
        used = min(current, remaining)
        setattr(factory, stock_field, current - used)
        remaining -= used

        if factory.owner and not factory.owner.is_dead:
            factory.owner.money = int(safe_number(factory.owner.money)) + (used * int(unit_price))

    return True, f"Bought {amount} units from local weapon factories in {city}."



def car_factory_vehicle_key(user):
    rank = normalize_rank_name(getattr(user, "rank", "Street Runner") or "Street Runner")
    for rank_name, vehicle_key in CAR_FACTORY_RANK_VEHICLES:
        if rank == rank_name:
            return vehicle_key
    return "birmingham_courier_bicycle"


def ensure_player_city_factories(user):
    if not user or not user.id:
        return []
    existing = {(row.city, row.factory_type): row for row in CityFactory.query.filter_by(user_id=user.id).all()}
    changed = False
    for city in CITIES:
        for factory_type in PRODUCTION_FACTORY_TYPES.keys():
            key = (city, factory_type)
            if key not in existing:
                existing[key] = CityFactory(
                    user_id=user.id,
                    city=city,
    ...
    if changed:
        db.session.commit()
    return [existing[(city, factory_type)] for city in CITIES for factory_type in PRODUCTION_FACTORY_TYPES.keys()]


def city_factory_row(user, city, factory_type):
    ensure_player_city_factories(user)
    row = CityFactory.query.filter_by(user_id=user.id, city=city, factory_type=factory_type).first()
    if not row:
        row = CityFactory(user_id=user.id, city=city, factory_type=factory_type, quantity=0, pending_quantity=0, last_production=time.time())
        db.session.add(row)
        db.session.commit()
    return row


def update_city_factory_production(row):
    if not row:
        return 0
    data = PRODUCTION_FACTORY_TYPES.get(row.factory_type)
    if not data:
        return 0

    now = time.time()
    qty = int(safe_number(row.quantity))
    if qty <= 0:
        row.last_production = now
        return 0

    if safe_number(row.last_production) <= 0:
        row.last_production = now
        return 0

    elapsed = int(now - safe_number(row.last_production))
    interval = int(data["interval"])
    ticks = elapsed // interval
    if ticks <= 0:
        return 0

    produced = ticks * qty * int(data.get("amount", 1))
    row.pending_quantity = int(safe_number(row.pending_quantity)) + produced
    row.last_production = safe_number(row.last_production) + (ticks * interval)
    return produced


def update_all_city_factories(user):
    rows = ensure_player_city_factories(user)
    for row in rows:
        update_city_factory_production(row)
    db.session.commit()
    return rows



def business_empire_summary(user):
    try:
        factories = production_factory_rows(user)
        owned = sum(f["quantity"] for c in factories for f in c["factories"])
        ready = sum(f["pending"] for c in factories for f in c["factories"])
    except Exception:
        owned = 0
        ready = 0
    ...
    return {
        "owned": owned,
        "ready": ready,
        "warehouse_used": warehouse_used(user),
        "warehouse_capacity": warehouse_capacity(user),
    }


def production_factory_rows(user):
    update_all_city_factories(user)
    rows = []
    for city in CITIES:
        city_rows = []
        for factory_type, data in PRODUCTION_FACTORY_TYPES.items():
            row = city_factory_row(user, city, factory_type)
            city_rows.append({
                "row": row,
                "type": factory_type,
                "name": data["name"],
                ...
                "quantity": int(safe_number(row.quantity)),
                "pending": int(safe_number(row.pending_quantity)),
            })
        rows.append({"city": city, "factories": city_rows})
    return rows


def collect_city_production(user, city):
    ensure_player_city_factories(user)
    collected_parts = []

    for row in CityFactory.query.filter_by(user_id=user.id, city=city).all():
        update_city_factory_production(row)
        data = PRODUCTION_FACTORY_TYPES.get(row.factory_type)
        if not data:
            continue
        ...
        if data["kind"] == "warehouse":
            ...
            if free_space <= 0:
                continue
            amount = min(pending, free_space)
            warehouse_row = warehouse_item(user, item_key)
            warehouse_row.quantity = int(safe_number(warehouse_row.quantity)) + amount
            row.pending_quantity = pending - amount
            collected_parts.append(f"{amount} {data['unit_label']}")

        elif data["kind"] == "vehicle":
            vehicle_key = car_factory_vehicle_key(user)
            vehicle = VEHICLE_BY_KEY.get(vehicle_key)
            for _ in range(pending):
                add_vehicle_to_city(user, city, vehicle_key, 1)
            row.pending_quantity = 0
            if vehicle:
                collected_parts.append(f"{pending}x {vehicle['year']} {vehicle['name']}")
...
def casino_city_stats(city):
    rows = casinos_for_city(city)
    owned = sum(1 for c in rows if c.owner_id)
    state_owned = sum(1 for c in rows if not c.owner_id)
    vault_total = sum(int(safe_number(c.vault)) for c in rows)
    return {
        "licenses": len(rows),
        "owned": owned,
...
def casino_game_rows():
    return [
        {"key": "coin", "icon": "🪙", "name": "Coin Flip", "tag": "Classic", "description": "50/50 style table. Win 2x your bet.", "default_bet": 100},
        {"key": "dice", "icon": "🎲", "name": "Dice Six", "tag": "5x", "description": "Roll a six and take 5x. High risk, clean payout.", "default_bet": 100},
        {"key": "slots", "icon": "🎰", "name": "Shelby Slots", "tag": "10x", "description": "Spin the reels for pairs or a full jackpot.", "default_bet": 100},
        {"key": "roulette", "icon": "🔴", "name": "Roulette", "tag": "Red/Black", "description": "Pick red or black. Green belongs to the house.", "default_bet": 100},
        {"key": "highcard", "icon": "🃏", "name": "High Card", "tag": "Duel", "description": "Draw higher than the dealer. Ties return the bet.", "default_bet": 100},
        {"key": "blackjack", "icon": "♠️", "name": "Blackjack", "tag": "3x BJ", "description": "Beat the dealer without going over 21. Blackjack pays 3x.", "default_bet": 250},
        {"key": "baccarat", "icon": "♦️", "name": "Baccarat", "tag": "9 wins", "description": "Closest hand to 9 wins. Elegant, fast and dangerous.", "default_bet": 250},
        {"key": "wheel", "icon": "🎡", "name": "Golden Wheel", "tag": "20x", "description": "Spin for 0x, 2x, 5x, 10x or a rare 20x prize.", "default_bet": 500},
        {"key": "lucky7", "icon": "7️⃣", "name": "Lucky Sevens", "tag": "7x", "description": "Roll two dice. Hit exactly 7 to win 7x.", "default_bet": 200},
    ]
...
/* Casino Luxury Rework */
.casino-lux-hero{
    position:relative;
    overflow:hidden;
    border:1px solid rgba(255,212,122,.32);
    border-radius:22px;
    padding:28px;
    margin-bottom:18px;
    background:
        radial-gradient(circle at 12% 10%, rgba(255,212,122,.22), transparent 30%),
        radial-gradient(circle at 85% 20%, rgba(130,18,18,.22), transparent 34%),
        linear-gradient(135deg, rgba(7,10,12,.98), rgba(35,18,10,.92));
    box-shadow:0 22px 55px rgba(0,0,0,.42), inset 0 0 40px rgba(255,212,122,.06);
}
.casino-lux-hero:after{
    content:"";
    position:absolute;
    inset:auto -20% -45% -20%;
    height:160px;
    background:linear-gradient(90deg, transparent, rgba(255,212,122,.18), transparent);
...
.casino-game-card{
    ...
    border-radius:18px;
    padding:18px;
    background:linear-gradient(145deg, rgba(9,13,15,.96), rgba(31,22,16,.88));
...
.casino-game-card:hover{
    ...
    border-color:rgba(255,212,122,.55);
    box-shadow:0 18px 45px rgba(217,154,43,.18);
}
.casino-game-top{
    display:flex;
...
.casino-form-row .input{min-width:110px}
.casino-owner-panel{
    border:1px solid rgba(255,212,122,.25);
    border-radius:18px;
    padding:18px;
    background:linear-gradient(135deg, rgba(0,0,0,.28), rgba(255,212,122,.07));
}
.casino-vip-table tr:hover{
...
.login-cinematic{
    ...
    background:
        ...
        radial-gradient(circle at 20% 12%, rgba(214,168,95,.24), transparent 28%),
...
<div class="login-wrap login-cinematic">
    ...
    <div class="login-card login-card-wide">
        ...
        <p>Enter the streets of Birmingham. Build your empire, own businesses, control casinos and rise through the ranks.</p>
        ...
        {% if page == "forgot_password" %}
            ...
            <div class="login-two-col">
                ...
                <form action="/reset_password" method="post" class="login-panel-form">
                    <h3>Set New Password</h3>
                    Email:<br><input class="input" type="email" name="email" required maxlength="160">
                    Reset code:<br><input class="input" name="reset_token" required maxlength="40">
                    New password:<br><input class="input" type="password" name="new_password" required minlength="3">
                    <button class="btn">Reset Password</button>
                </form>
            </div>
        {% else %}
            <div class="login-two-col">
                <form action="/login_action" method="post" class="login-panel-form">
                    <h3>Login</h3>
                    Username or email:<br><input class="input" name="username" required maxlength="160">
                    Password:<br><input class="input" type="password" name="password" required>
                    <button class="btn" name="action" value="login">Login</button>
                    <a class="login-link" href="/forgot_password">Forgot password?</a>
                </form>

                <form action="/login_action" method="post" class="login-panel-form">
                    <h3>Create Account</h3>
                    Username:<br><input class="input" name="username" required maxlength="80">
                    Email:<br><input class="input" type="email" name="email" required maxlength="160">
                    Password:<br><input class="input" type="password" name="password" required minlength="3">
                    <button class="btn" name="action" value="register">Create Account</button>
                </form>
            </div>
        {% endif %}
    </div>
...
<div class="app-shell">
    <aside class="sidebar">
        ...
        <div class="nav grouped-nav">
            ...
            <div class="menu-section">
                ...
                <a href="/marketplace" class="{% if page == 'marketplace' %}active{% endif %}">Marketplace</a>
        ...
        <div class="s3-dashboard s-bank-page">
            ...
            <div class="s3-grid-3">
                ...
                <article class="s3-panel">
                    ...
                    <div class="s3-mini-list">
                        <div><span>Send from</span><b>Bank Balance</b></div>
                        <div><span>Your bank balance</span><b class="gold">${{ moneyfmt(user.bank) }}</b></div>
                    </div>
                    <form action="/bank_action" method="post" class="s-bank-form">
                        Username:<br>
                        <input class="input" name="target_username" placeholder="Username" required>
                        Amount:<br>
                        <input class="input" type="number" name="amount" min="1" placeholder="Amount" required>
                        <div class="s-bank-actions">
                            <button class="btn" name="action" value="transfer_player">Send Money</button>
                        </div>
                    </form>
                    <p class="s4-note">Money is sent directly from your bank balance to the other player's bank balance.</p>
                </article>
            </div>
        </div>
    {% endif %}
    ...
    {% if page == "market" %}
        ...
        <div class="card">
            <h3>🔫 Weapons in Smuggling</h3>
            <p><b class="gold">Weapon Crates</b> are smuggling goods. They are not the same as usable weapons from the Weapons page.</p>
            <p>Use smuggling to trade crates for profit. Use the Weapons page to buy usable weapons that increase your weapon power.</p>
            {% set weapon_route = best_smuggling_route("weapons") %}
            {% if weapon_route %}
                <p>Best current weapon-crate route: <b class="gold">{{ weapon_route.buy_city }}</b> → <b class="gold">{{ weapon_route.sell_city }}</b>
                | Estimated profit per crate: <b class="good">${{ moneyfmt(weapon_route.profit) }}</b></p>
            {% endif %}
        </div>

        <div class="card">
            <h3>⚔️ Your Usable Weapons</h3>
            <p>These are your real weapons. They are separate from smuggled Weapon Crates.</p>
            <table>
                <tr><th>Weapon</th><th>Owned</th><th>Power Each</th><th>Total Power</th></tr>
                {% for weapon in weapon_inventory_rows(user) %}
                    <tr>
                        <td>{{ weapon.name }}</td>
                        <td>{{ weapon.quantity }}</td>
                        <td class="gold">+{{ weapon.attack }}</td>
                        <td class="gold">{{ moneyfmt(weapon.total_attack) }}</td>
                    </tr>
                {% endfor %}
            </table>
            <a class="btn" href="/bullets">Open Weapons Market</a>
        </div>
        ...
        <table>
            ...
            <tr>
                <td>{{ item.label }}</td>
                <td>{% if item.key == "weapons" %}<span class="red">Contraband Crates</span>{% else %}<span class="muted">Trade Goods</span>{% endif %}</td>
        ...
        <p class="muted">Prices update every hour. Weapon Crates are high-risk contraband with higher profit potential, while usable weapons are managed on the Weapons page.</p>
        <table>
            <tr>
                <th>City</th>
    ...
    {% if page == "marketplace" %}
        <h2>Shelby Exchange Marketplace</h2>
        <p class="muted">Trade goods, bullets, weapons and vehicles with other players. Creating a listing costs a 2% marketplace tax paid into the State Treasury.</p>

        <div class="s3-stat-grid">
            <div class="s3-stat"><div class="icon">📋</div><small>Active Listings</small><b>{{ moneyfmt(marketplace_stats_data.active_listings) }}</b></div>
            <div class="s3-stat"><div class="icon">💰</div><small>Marketplace Value</small><b>${{ moneyfmt(marketplace_stats_data.marketplace_value) }}</b></div>
            <div class="s3-stat"><div class="icon">🔁</div><small>Trades Today</small><b>{{ moneyfmt(marketplace_stats_data.trades_today) }}</b></div>
            <div class="s3-stat"><div class="icon">🏦</div><small>State Treasury</small><b>${{ moneyfmt(marketplace_stats_data.treasury) }}</b></div>
        </div>

        <div class="card">
            <h2>🔎 Search & Filter</h2>
            <form action="/marketplace" method="get" style="display:flex;gap:10px;flex-wrap:wrap;align-items:end">
                <div>
                    Type<br>
                    <select class="input" name="type">
                        <option value="all" {% if marketplace_type == "all" %}selected{% endif %}>All</option>
                        <option value="weapon" {% if marketplace_type == "weapon" %}selected{% endif %}>Weapons</option>
                        <option value="bullets" {% if marketplace_type == "bullets" %}selected{% endif %}>Bullets</option>
                        <option value="vehicle" {% if marketplace_type == "vehicle" %}selected{% endif %}>Vehicles</option>
                        <option value="goods" {% if marketplace_type == "goods" %}selected{% endif %}>Smuggling Goods</option>
                    </select>
                </div>
                <div>
                    Search player or item<br>
                    <input class="input" name="q" value="{{ marketplace_query or '' }}" placeholder="Tommy, Revolver, Whiskey, Bentley">
                </div>
                <button class="btn">Search</button>
                <a class="btn" href="/marketplace">Reset</a>
            </form>
            <p class="muted">Most traded item: <b class="gold">{{ marketplace_stats_data.most_traded }}</b></p>
        </div>

        <div class="card">
            <h2>➕ Create Listing</h2>
            <p class="muted">The item is removed from your inventory when you list it. If you cancel, remaining stock returns to you.</p>

            <h3>💥 Bullets</h3>
            <form action="/marketplace_create" method="post" style="display:flex;gap:10px;flex-wrap:wrap;align-items:end">
                <input type="hidden" name="item_type" value="bullets">
                <input type="hidden" name="item_key" value="bullets">
                <div>Available<br><b class="gold">{{ create_options.bullets }}</b></div>
                <div>Quantity<br><input class="input" type="number" name="quantity" min="1" max="{{ create_options.bullets }}" value="100"></div>
                <div>Price each<br><input class="input" type="number" name="price_each" min="1" value="25"></div>
                <button class="btn">List Bullets</button>
            </form>

            <hr>

            <h3>📦 Smuggling Goods</h3>
            <table>
                <tr><th>Goods</th><th>Owned</th><th>Create Listing</th></tr>
                {% for item in create_options.goods %}
                <tr>
                    <td>{{ item.label }}</td>
                    <td>{{ item.quantity }}</td>
                    <td>
                        <form action="/marketplace_create" method="post">
                            <input type="hidden" name="item_type" value="goods">
                            <input type="hidden" name="item_key" value="{{ item.key }}">
                            Qty <input class="input" style="width:90px" type="number" name="quantity" min="1" max="{{ item.quantity }}" value="1">
                            Price <input class="input" style="width:120px" type="number" name="price_each" min="1" value="100">
            ...
            <h3>🔫 Weapons</h3>
            <table>
                <tr><th>Weapon</th><th>Owned</th><th>Create Listing</th></tr>
                {% for item in create_options.weapons %}
                <tr>
                    <td>{{ item.label }}</td>
                    <td>{{ item.quantity }}</td>
                    <td>
                        <form action="/marketplace_create" method="post">
                            <input type="hidden" name="item_type" value="weapon">
                            <input type="hidden" name="item_key" value="{{ item.key }}">
                            Qty <input class="input" style="width:90px" type="number" name="quantity" min="1" max="{{ item.quantity }}" value="1">
                            Price <input class="input" style="width:120px" type="number" name="price_each" min="1" value="1000">
                            <button class="btn">List</button>
                        </form>
                    </td>
                </tr>
            ...
            <h3>🚗 Vehicles in {{ user.location }}</h3>
            <table>
                <tr><th>Vehicle</th><th>Owned Here</th><th>Create Listing</th></tr>
                {% for item in create_options.vehicles %}
                <tr>
                    <td>{{ item.label }}</td>
                    <td>{{ item.quantity }}</td>
                    <td>
        ...
        <table>
            <tr><th>Seller</th><th>Type</th><th>Item</th><th>City</th><th>Qty</th><th>Price Each</th><th>Total</th><th>Action</th></tr>
            {% for listing in listings %}
            <tr>
                <td>{{ listing.seller.username if listing.seller else "Unknown" }}</td>
                <td>{{ listing.item_type|title }}</td>
                <td>{{ marketplace_item_label(listing.item_type, listing.item_key) }}</td>
                <td>{% if listing.city %}{{ listing.city }}{% else %}-{% endif %}</td>
                <td>{{ listing.quantity }}</td>
                <td class="gold">${{ moneyfmt(listing.price_each) }}</td>
                <td class="good">${{ moneyfmt(listing.price_each * listing.quantity) }}</td>
                <td>
                    {% if listing.seller_id == user.id %}
                        <form action="/marketplace_cancel" method="post">
                            <input type="hidden" name="listing_id" value="{{ listing.id }}">
                            <button class="btn btn-delete">Cancel</button>
                        </form>
                    {% else %}
                        <form action="/marketplace_buy" method="post">
                            <input type="hidden" name="listing_id" value="{{ listing.id }}">
                            Qty <input class="input" style="width:80px" type="number" name="quantity" min="1" max="{{ listing.quantity }}" value="{{ listing.quantity }}">
        ...
        <h2>📜 Recent Trades</h2>
        <table>
            <tr><th>Time</th><th>Buyer</th><th>Seller</th><th>Item</th><th>Qty</th><th>Total</th></tr>
            {% for trade in recent_trades %}
            <tr>
                <td>{{ message_age(trade) }}</td>
                <td>{{ trade.buyer.username if trade.buyer else "Unknown" }}</td>
                <td>{{ trade.seller.username if trade.seller else "Unknown" }}</td>
                <td>{{ marketplace_item_label(trade.item_type, trade.item_key) }}</td>
                <td>{{ trade.quantity }}</td>
                <td class="good">${{ moneyfmt(trade.total_price) }}</td>
            </tr>
            {% endfor %}
        </table>
    {% endif %}
    ...
    {% if page == "assets" %}
        {% set empire = business_empire_summary(user) %}
        <section class="s5-hero">
            <div class="s5-eyebrow">Business Empire</div>
            <h1>🏭 BUSINESSES</h1>
            <p>Manage production, factories, local stock, warehouse goods, weapon supply and vehicle output from one clear dashboard.</p>
        </section>

        <div class="s3-stat-grid">
            <div class="s3-stat"><div class="icon">📍</div><small>Current City</small><b>{{ user.location }}</b></div>
            <div class="s3-stat"><div class="icon">🏭</div><small>Factories Owned</small><b>{{ empire.owned }}</b></div>
            <div class="s3-stat"><div class="icon">📦</div><small>Ready Production</small><b>{{ empire.ready }}</b></div>
            <div class="s3-stat"><div class="icon">🏚</div><small>Warehouse</small><b>{{ empire.warehouse_used }}/{{ empire.warehouse_capacity }}</b></div>
        </div>

        <div class="card">
            <h2>📦 Ready To Collect in {{ user.location }}</h2>
            <p>Use this first. It collects all ready local factory production. Warehouse goods need free warehouse space. Cars go directly to the local garage.</p>
            <form action="/collect_factory_action" method="post">
                <button class="btn">Collect All Local Production</button>
            </form>
        </div>

        <hr>
        <h2>🛒 Buy Businesses in {{ user.location }}</h2>
        <p class="muted">All businesses are city-bound. Buy factories in the city where you want them to produce.</p>

        <h3>🥃 Alcohol Production</h3>
        <div class="grid">
            <div class="card">
                <h3>Gin Distillery</h3>
                <p>Cost: <b class="gold">$2,500</b></p>
                <p>Produces gin every minute in this city.</p>
                <form action="/asset_action" method="post">
                    <button class="btn" name="action" value="buy_distillery">Buy Gin Distillery</button>
                </form>
            </div>
            {% for factory_type in ["whiskey_distillery", "rum_distillery"] %}
                {% set data = production_factory_types[factory_type] %}
                <div class="card">
                    <h3>{{ data.name }}</h3>
                    <p>Cost: <b class="gold">${{ moneyfmt(data.cost) }}</b></p>
                    <p>Produces <b>{{ data.unit_label }}</b> every {{ format_duration(data.interval) }}.</p>
                    <form action="/asset_action" method="post">
                        <input type="hidden" name="factory_type" value="{{ factory_type }}">
                        <button class="btn" name="action" value="buy_production_factory">Buy Factory</button>
                    </form>
                </div>
            {% endfor %}
        </div>
        ...
        <h3>🚬 Luxury Goods</h3>
        <div class="grid">
            {% for factory_type in ["cigar_factory", "luxury_watches_factory"] %}
                {% set data = production_factory_types[factory_type] %}
                <div class="card">
                    <h3>{{ data.name }}</h3>
                    <p>Cost: <b class="gold">${{ moneyfmt(data.cost) }}</b></p>
        ...
        <h3>🔫 Weapons Supply</h3>
        <div class="grid">
            {% set data = production_factory_types["weapon_crates_factory"] %}
            <div class="card">
                <h3>{{ data.name }}</h3>
                <p>Cost: <b class="gold">${{ moneyfmt(data.cost) }}</b></p>
                <p>Produces <b>{{ data.unit_label }}</b> every {{ format_duration(data.interval) }} for Smuggling.</p>
                <form action="/asset_action" method="post">
                    <input type="hidden" name="factory_type" value="weapon_crates_factory">
                    <button class="btn" name="action" value="buy_production_factory">Buy Factory</button>
                </form>
            </div>

            {% for row in local_weapon_factories %}
            <div class="card">
                <h3>{{ row.factory.name }}</h3>
                <p>Owner: {% if row.is_state %}<span class="muted">State</span>{% else %}<span class="gold">{{ row.owner }}</span>{% endif %}</p>
                <p>Bullets: <b class="gold">{{ moneyfmt(row.bullet_stock) }}</b> | Weapons: <b class="gold">{{ moneyfmt(row.weapon_stock) }}</b></p>
                <p>Price: <b class="gold">${{ moneyfmt(row.price) }}</b></p>
                {% if row.is_state %}
                    <form action="/asset_action" method="post">
                        <input type="hidden" name="slot_number" value="{{ row.factory.slot_number }}">
                        <button class="btn" name="action" value="buy_weapon_factory">Buy Weapon Factory</button>
                    </form>
                {% elif row.factory.owner_id == user.id %}
                    <span class="good">Owned by you</span>
                {% else %}
                    <span class="red">Owned by another player</span>
                {% endif %}
            </div>
            {% endfor %}
        </div>
        ...
        <div class="grid">
            {% set data = production_factory_types["car_factory"] %}
            {% set vehicle_key = car_factory_vehicle_key(user) %}
            <div class="card">
                <h3>{{ data.name }}</h3>
                <p>Cost: <b class="gold">${{ moneyfmt(data.cost) }}</b></p>
                <p>Produces 1 vehicle every {{ format_duration(data.interval) }}.</p>
                <p>Your current rank produces: <b class="gold">{{ vehicle_by_key[vehicle_key].name if vehicle_key in vehicle_by_key else vehicle_key }}</b></p>
                <form action="/asset_action" method="post">
                    <input type="hidden" name="factory_type" value="car_factory">
                    <button class="btn" name="action" value="buy_production_factory">Buy Car Factory</button>
                </form>
            </div>
        </div>

        <hr>
        <h2>🏙 Current City Businesses</h2>
        <p class="muted">Everything you own in {{ user.location }} and what is ready locally.</p>
        <table>
            <tr><th>Business</th><th>Owned</th><th>Ready</th><th>Output</th><th>Production Time</th></tr>
            <tr>
                <td>Gin Distillery</td>
                <td>{{ current_business.distilleries }}</td>
                <td class="gold">{{ business_ready_gin(current_business) }} gin</td>
                <td>Gin</td>
                <td>1m</td>
            </tr>
            {% for cityrow in production_factories %}
                {% if cityrow.city == user.location %}
                    {% for factory in cityrow.factories %}
                    <tr>
                        <td>{{ factory.name }}</td>
                        <td>{{ factory.quantity }}</td>
                        <td class="gold">{{ factory.pending }}</td>
                        <td>{{ factory.unit_label }}</td>
                        <td>{{ format_duration(factory.interval) }}</td>
                    </tr>
                    {% endfor %}
                {% endif %}
            {% endfor %}
        ...
        <h2>🤝 Local Buyer</h2>
        <div class="card">
            <h3>Sell Local Gin in {{ user.location }}</h3>
            <p>Local buyer price: <b class="good">${{ local_buyer_gin_price(user) }}</b> per gin. This is 30% below the current city's smuggling price of <b class="gold">${{ local_smuggling_gin_price(user) }}</b>.</p>
        ...
        <p class="muted">Full city-by-city overview. Travel to a city to collect local production there.</p>
        <table>
            <tr><th>City</th><th>Gin Distilleries</th><th>Ready Gin</th><th>Other Factories</th><th>Ready Other Production</th><th>Status</th></tr>
            {% for business in businesses %}
                {% set other_count = 0 %}
                {% set other_ready = 0 %}
                {% for cityrow in production_factories %}
                    {% if cityrow.city == business.city %}
                        {% for factory in cityrow.factories %}
                            {% set other_count = other_count + factory.quantity %}
                            {% set other_ready = other_ready + factory.pending %}
                        {% endfor %}
                    {% endif %}
                {% endfor %}
                <tr>
                    ...
                    <td class="gold">{{ business_ready_gin(business) }}</td>
                    <td>{{ other_count }}</td>
                    <td class="gold">{{ other_ready }}</td>
                    <td>{% if business.city == user.location %}<span class="good">You are here</span>{% else %}<span class="red">Travel here to collect</span>{% endif %}</td>
                </tr>
            {% endfor %}
        </table>
    {% endif %}
    ...
    {% if page == "bullets" %}
        ...
        <div class="card">
            <h3>Buy Ammunition in {{ user.location }}</h3>
            <p>Local factory bullet stock: <b class="gold">{{ moneyfmt(total_factory_bullet_stock(user.location)) }}</b></p>
            <p class="muted">Bullets cost ${{ bullet_unit_price }} each. Quick buy is still available, but you can also enter your own amount.</p>
            <form action="/bullet_action" method="post" style="display:flex;gap:10px;flex-wrap:wrap;align-items:end">
                <button class="btn" name="action" value="buy_bullet">Buy 10 bullets - $200</button>
            </form>
            <form action="/bullet_action" method="post" style="display:flex;gap:10px;flex-wrap:wrap;align-items:end;margin-top:10px">
                <div>
                    Amount:<br>
                    <input class="input" type="number" name="amount" min="1" value="50">
                </div>
                <button class="btn" name="action" value="buy_bullet">Buy Custom Amount</button>
            </form>
        </div>

        <div class="card">
            <h3>Local Weapon Factories</h3>
            <p class="muted">Every purchase is supplied by factories in {{ user.location }}. Owner receives the money; State-owned factories remove money from circulation.</p>
            <table>
                <tr><th>Factory</th><th>Owner</th><th>Bullets</th><th>Weapons</th></tr>
                {% for row in weapon_factory_overview_rows(user.location) %}
                <tr>
                    <td>{{ row.factory.name }}</td>
                    <td>{% if row.is_state %}<span class="muted">State</span>{% else %}<span class="gold">{{ row.owner }}</span>{% endif %}</td>
                    <td>{{ moneyfmt(row.bullet_stock) }}</td>
                    <td>{{ moneyfmt(row.weapon_stock) }}</td>
                </tr>
                {% endfor %}
            </table>
        </div>
            ...
            <article class="s4-operation-card">
                ...
                <div class="s4-card-body">
                    <div class="s4-metrics">
                        ...
                        <div class="s4-metric"><small>Local Stock</small><b>{{ total_factory_weapon_stock(user.location, weapon.key) }}</b></div>
                    ...
                    <form action="/bullet_action" method="post">
                        ...
                        Amount:<br><input class="input" type="number" name="amount" min="1" value="1">
    ...
    {% if page == "casino" %}
        {% set city_stats = casino_city_stats(user.location) %}
        <section class="casino-lux-hero">
            <div class="casino-lux-eyebrow">Shelby Private Gaming Rooms</div>
            <h1>CASINO ROYALE</h1>
            <p>Step into the luxury tables of {{ user.location }}. Play fast games, chase jackpots, own city casinos and collect the house cut from every lost bet in your city.</p>
        </section>

        <div class="s3-stat-grid">
            <div class="s3-stat"><div class="icon">📍</div><small>City</small><b>{{ user.location }}</b></div>
            <div class="s3-stat"><div class="icon">🏛</div><small>City Licenses</small><b>{{ city_stats.licenses }}</b></div>
            <div class="s3-stat"><div class="icon">💰</div><small>City Vaults</small><b>${{ moneyfmt(city_stats.vault_total) }}</b></div>
            <div class="s3-stat"><div class="icon">👑</div><small>Your Casinos</small><b>{{ user_casinos(user)|length }}</b></div>
        </div>

        <div class="casino-owner-panel">
            <h2>💎 House Rules</h2>
            <p>Casino owners receive <b class="gold">{{ house_cut }}%</b> from lost bets in their city. Higher casino ownership means steady passive income inside the casino vault.</p>
            <p class="muted">Luxury rooms are riskier than street crimes, but the payouts are immediate and the casino economy stays alive through player-owned licenses.</p>
        </div>

        <hr>
        <h2>🎲 Luxury Casino Games</h2>
        <div class="casino-game-grid">
            {% for game in casino_game_rows() %}
            <article class="casino-game-card">
                <div class="casino-game-top">
                    <div style="display:flex;gap:12px">
                        ...
                        <div>
                            <h3>{{ game.name }}</h3>
                            <span class="casino-tag">{{ game.tag }}</span>
                        </div>
                    </div>
                </div>
                ...
                <form action="/casino_action" method="post">
                    <div class="casino-form-row">
                        <div>
                            Bet:<br>
                            <input class="input" type="number" name="bet" min="1" value="{{ game.default_bet }}" required>
                        </div>
                        ...
                        {% endif %}
                        <button class="btn" name="game" value="{{ game.key }}">Play</button>
                    </div>
                </form>
            </article>
            {% endfor %}
        ...
        <p class="muted">Buy a casino to earn from the house cut. If an owner dies, ownership can move through the heir system.</p>
        <table class="casino-vip-table">
            ...
            <tr>
                ...
                <td class="good">${{ moneyfmt(casino.price) }}</td>
                <td class="gold">${{ moneyfmt(casino.vault) }}</td>
                <td>
                    {% if casino.owner_id == user.id %}
        ...
        <hr>
        <h2>🌍 Casino Empire Overview</h2>
        <table class="casino-vip-table">
            <tr><th>City</th><th>Casino</th><th>Status</th><th>Price</th><th>Vault</th></tr>
...
def render_page(user, page, **kwargs):
    return render_template_string(
        ...
        business_empire_summary=business_empire_summary,
        ...
        best_smuggling_route=best_smuggling_route,
        marketplace_item_label=marketplace_item_label,
        marketplace_fee_rate=MARKETPLACE_FEE_RATE,
        marketplace_stats=marketplace_stats,
        recent_marketplace_trades=recent_marketplace_trades,
        city_weapon_factories=city_weapon_factories,
        weapon_factory_overview_rows=weapon_factory_overview_rows,
        total_factory_bullet_stock=total_factory_bullet_stock,
        total_factory_weapon_stock=total_factory_weapon_stock,
        bullet_unit_price=BULLET_UNIT_PRICE,
        weapon_factory_slots_per_city=WEAPON_FACTORY_SLOTS_PER_CITY,
        production_factory_rows=production_factory_rows,
        production_factory_types=PRODUCTION_FACTORY_TYPES,
        car_factory_vehicle_key=car_factory_vehicle_key,
        collect_city_production=collect_city_production,
        ...
        casino_city_stats=casino_city_stats,
        casino_game_rows=casino_game_rows,
...
def login_action():
    ...
    if action == "register":
        if "@" not in email or "." not in email:
            return redirect(url_for("index", msg="Enter a valid email address."))
        if User.query.filter_by(username=username).first():
            return redirect(url_for("index", msg="That username already exists."))
        if User.query.filter(User.email.ilike(email)).first():
            return redirect(url_for("index", msg="That email address is already registered."))
...
def forgot_password():
    return render_page(None, "forgot_password", msg=request.args.get("msg"))
...
def reset_password():
    ...
    if len(new_password) < 3:
        return redirect(url_for("forgot_password", msg="New password must be at least 3 characters."))

    user = User.query.filter(User.email.ilike(email)).first()
    if not user or not user.reset_token or user.reset_token != token:
        return redirect(url_for("forgot_password", msg="Invalid reset code."))
    if safe_number(user.reset_expires_at) < time.time():
        return redirect(url_for("forgot_password", msg="Reset code expired. Request a new one."))

    user.password_hash = generate_password_hash(new_password)
    user.reset_token = None
    user.reset_expires_at = 0.0
    db.session.commit()
    return redirect(url_for("index", msg="Password reset successful. You can now log in."))
...
def crime_action():
    ...
    if crime_key == "weapon_theft":
        ...
        if weapon:
            ...
            user.exp += exp
            msg = f"🔫 Weapon raid successful. You stole a usable {weapon['name']} and found {bullets_found} bullets. +{exp} EXP."
        else:
...
def bank_action():
    ...
    if action == "deposit":
        ...
        if not target_username:
            return redirect(url_for("bank", msg="Enter a username."))
        if target_username.lower() == user.username.lower():
            return redirect(url_for("bank", msg="You cannot send money to yourself."))
        receiver = User.query.filter(User.username.ilike(target_username)).first()
        if not receiver or receiver.is_dead:
            return redirect(url_for("bank", msg="Player not found."))
        if safe_number(user.bank) < amount:
            return redirect(url_for("bank", msg="Not enough money in your bank balance."))
        user.bank = int(safe_number(user.bank)) - amount
        receiver.bank = int(safe_number(receiver.bank)) + amount
        create_message(receiver.id, "system", "Bank Transfer Received", f"{user.username} sent you ${amount} through the bank.", commit=False)
        create_message(user.id, "system", "Bank Transfer Sent", f"You sent ${amount} to {receiver.username}.", commit=False)
        msg = f"You sent ${amount} to {receiver.username}."
    elif action == "borrow":
...
def marketplace():
    ...
    item_type = request.args.get("type", "all").strip() or "all"
    search_query = request.args.get("q", "").strip()
    listings = marketplace_filtered_listings(item_type=item_type, query=search_query)

    return render_page(
        user,
        "marketplace",
        listings=listings,
        create_options=marketplace_create_options(user),
        marketplace_stats_data=marketplace_stats(listings),
        marketplace_type=item_type,
        marketplace_query=search_query,
        recent_trades=recent_marketplace_trades(25),
        msg=request.args.get("msg"),
    )


@app.route("/marketplace_create", methods=["POST"])
def marketplace_create():
    ...
    price_each = safe_int(request.form.get("price_each"))

    if quantity <= 0 or price_each <= 0:
        return redirect(url_for("marketplace", msg="Invalid quantity or price."))

    total_value = quantity * price_each
    fee = marketplace_listing_fee(total_value)
    if safe_number(user.money) < fee:
        return redirect(url_for("marketplace", msg=f"Not enough cash for listing fee: ${fee}."))

    ok, reason = marketplace_remove_from_seller(user, item_type, item_key, quantity)
    if not ok:
        return redirect(url_for("marketplace", msg=reason))

    user.money = int(safe_number(user.money)) - fee
    add_state_treasury(fee)
    listing = MarketplaceListing(
        seller_id=user.id,
        item_type=item_type,
        item_key=item_key,
        city=user.location if item_type == "vehicle" else None,
        quantity=quantity,
    ...
    db.session.commit()
    return redirect(url_for("marketplace", msg=f"Listing created. Fee paid: ${fee}."))


@app.route("/marketplace_buy", methods=["POST"])
def marketplace_buy():
    user, error = login_required()
    if error:
        return error

    listing_id = safe_int(request.form.get("listing_id"))
    amount = safe_int(request.form.get("quantity"))
    listing = MarketplaceListing.query.get(listing_id)

    if not listing or not listing.is_active or safe_number(listing.quantity) <= 0:
        return redirect(url_for("marketplace", msg="Listing not found."))
    if listing.seller_id == user.id:
        return redirect(url_for("marketplace", msg="You cannot buy your own listing."))
    if amount <= 0 or amount > int(safe_number(listing.quantity)):
        return redirect(url_for("marketplace", msg="Invalid amount."))

    total = amount * int(safe_number(listing.price_each))
    if safe_number(user.money) < total:
        return redirect(url_for("marketplace", msg="Not enough cash."))

    seller = User.query.get(listing.seller_id)
    user.money = int(safe_number(user.money)) - total
    if seller and not seller.is_dead:
        seller.money = int(safe_number(seller.money)) + total
        create_message(seller.id, "system", "Marketplace Sale", f"{user.username} bought {amount}x {marketplace_item_label(listing.item_type, listing.item_key)} for ${total}.", commit=False)

    marketplace_give_item(user, listing.item_type, listing.item_key, amount, listing.city)
    trade = MarketplaceTrade(
        ...
        quantity=amount,
        price_each=int(safe_number(listing.price_each)),
        total_price=total,
        created_at=time.time(),
    )
    db.session.add(trade)

    listing.quantity = int(safe_number(listing.quantity)) - amount
    if listing.quantity <= 0:
        listing.is_active = False

    db.session.commit()
    return redirect(url_for("marketplace", msg=f"Purchase complete: {amount}x {marketplace_item_label(listing.item_type, listing.item_key)}."))


@app.route("/marketplace_cancel", methods=["POST"])
def marketplace_cancel():
    ...
    if not listing or listing.seller_id != user.id or not listing.is_active:
        return redirect(url_for("marketplace", msg="Listing not found."))

    remaining = int(safe_number(listing.quantity))
    if remaining > 0:
        marketplace_give_item(user, listing.item_type, listing.item_key, remaining, listing.city)
    listing.is_active = False
    listing.quantity = 0
    db.session.commit()
    return redirect(url_for("marketplace", msg="Listing cancelled and remaining stock returned."))
...
def assets():
    ...
    city_weapon_factories(user.location)
    update_all_city_factories(user)
    return render_page(
        user,
        "assets",
        ...
        local_weapon_factories=weapon_factory_overview_rows(user.location),
        production_factories=production_factory_rows(user),
        msg=request.args.get("msg"),
    )
...
def asset_action():
    ...
    elif action == "buy_weapon_factory":
        ensure_city_weapon_factories()
        slot_number = safe_int(request.form.get("slot_number"))
        factory = CityWeaponFactory.query.filter_by(city=user.location, slot_number=slot_number).first()
        if not factory:
            return redirect(url_for("assets", msg="Invalid weapon factory."))
        if factory.owner_id and factory.owner and not factory.owner.is_dead:
            return redirect(url_for("assets", msg="This weapon factory is already owned by another player."))
        price = int(safe_number(factory.price))
        if safe_number(user.money) < price:
            return redirect(url_for("assets", msg="Not enough cash."))
        user.money = int(safe_number(user.money)) - price
        factory.owner_id = user.id
        msg = f"You bought {factory.name}. It will produce bullets and weapons for this city."

    elif action == "buy_production_factory":
        factory_type = request.form.get("factory_type", "")
        data = PRODUCTION_FACTORY_TYPES.get(factory_type)
        if not data:
            return redirect(url_for("assets", msg="Invalid factory type."))
        price = int(data["cost"])
        if safe_number(user.money) < price:
            return redirect(url_for("assets", msg="Not enough cash."))
        row = city_factory_row(user, user.location, factory_type)
        user.money = int(safe_number(user.money)) - price
        row.quantity = int(safe_number(row.quantity)) + 1
        if safe_number(row.last_production) <= 0:
            row.last_production = time.time()
        msg = f"You bought a {data['name']} in {user.location}."
...
@app.route("/collect_factory_action", methods=["POST"])
def collect_factory_action():
    user, error = login_required()
    if error:
        return error

    parts = collect_city_production(user, user.location)
    if not parts:
        return redirect(url_for("assets", msg=f"No factory production ready in {user.location}, or your warehouse is full."))
    return redirect(url_for("assets", msg="Collected: " + ", ".join(parts)))
...
def bullet_action():
    ...
    if action == "buy_bullet":
        amount = safe_int(request.form.get("amount")) or 10
        if amount <= 0:
            return redirect(url_for("bullets", msg="Invalid amount."))
        ok, msg = consume_factory_stock(user, user.location, "bullet_stock", amount, BULLET_UNIT_PRICE)
        if not ok:
            return redirect(url_for("bullets", msg=msg))
        user.bullets = int(safe_number(user.bullets)) + amount
        msg = f"You bought {amount} bullets from local weapon factories for ${amount * BULLET_UNIT_PRICE}."

    elif action == "buy_weapon":
        weapon_key = request.form.get("weapon_key", "")
        amount = safe_int(request.form.get("amount")) or 1
        ...
        ok, msg = consume_factory_stock(user, user.location, weapon_stock_field(weapon_key), amount, price)
        if not ok:
            return redirect(url_for("bullets", msg=msg))
        add_weapon(user, weapon_key, amount)
        msg = f"You bought {amount}x {weapon['name']} from local weapon factories."
...
def casino_action():
    ...
    if game == "coin":
        ...
        player_total = random.randint(16, 23)
        dealer_total = random.randint(16, 23)
        if player_total == 21:
            payout = bet * 3
            msg = f"Blackjack! You hit 21. Dealer had {dealer_total}. You won ${payout - bet} profit."
        elif player_total > 21:
            msg = f"Blackjack: you busted with {player_total}. Dealer had {dealer_total}. You lost ${bet}."
        elif dealer_total > 21 or player_total > dealer_total:
            payout = bet * 2
            msg = f"Blackjack: you had {player_total}, dealer had {dealer_total}. You won ${payout - bet} profit."
        elif player_total == dealer_total:
            payout = bet
            msg = f"Blackjack push: both had {player_total}. Your bet was returned."
        else:
            msg = f"Blackjack: you had {player_total}, dealer had {dealer_total}. You lost ${bet}."

    elif game == "baccarat":
        player_hand = random.randint(0, 9)
        banker_hand = random.randint(0, 9)
        if player_hand > banker_hand:
            payout = bet * 2
            msg = f"Baccarat: you drew {player_hand}, banker drew {banker_hand}. You won ${payout - bet} profit."
        elif player_hand == banker_hand:
            payout = bet
            msg = f"Baccarat tie at {player_hand}. Your bet was returned."
        else:
            msg = f"Baccarat: you drew {player_hand}, banker drew {banker_hand}. You lost ${bet}."

    elif game == "wheel":
        multiplier = random.choices([0, 2, 5, 10, 20], weights=[54, 30, 11, 4, 1], k=1)[0]
        if multiplier > 0:
            payout = bet * multiplier
            msg = f"Golden Wheel landed on {multiplier}x. You won ${payout - bet} profit."
        else:
            msg = f"Golden Wheel landed on 0x. You lost ${bet}."
        ...
        if total_roll == 7:
            payout = bet * 7
            msg = f"Lucky Sevens: {d1} + {d2} = 7. You won ${payout - bet} profit."
        else:
            msg = f"Lucky Sevens: {d1} + {d2} = {total_roll}. You lost ${bet}."
    else:
        user.money = int(safe_number(user.money)) + bet
