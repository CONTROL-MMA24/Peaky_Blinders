...
def reduce_loss_by_protection(user, amount):
    ...
    return max(0, int(amount * (100 - reduction) / 100))
...
def ensure_city_casinos():
    ...
    changed = False
    for city in MARKET_PRICES.keys():
        for slot in range(1, CASINOS_PER_CITY + 1):
...
def casino_house_income(city, amount):
    ...
    cut = max(1, amount * CASINO_HOUSE_CUT_PERCENT // 100)
...
def bribe_chance(user):
    ...
    return RANK_BRIBE_CHANCES.get(rank, 25)
...
<div class="main">
    ...
    {% if page == "casino" %}
        <h2>🃏 City Casinos</h2>
        <p>Each city has only <b class="gold">{{ casinos_per_city }}</b> casino licenses. Casinos can be bought from the State, from other players, or from an heir if the owner is dead.</p>
        <p>Casino owners receive a house cut from lost bets in their city. House cut: <b class="gold">{{ house_cut }}%</b>.</p>
...
def market_action():
    ...
    if action == "buy":
        ...
        base = price * amount
        bonus = base * (rank_bonus(user, "smuggling") + family_bonus(user, "smuggling")) // 100
        earned = base + bonus
        risk = max(0, int(item_data["risk"]) - lookout_arrest_reduction(user))
        ...
        if random.randint(1, 100) <= risk:
            ...
            jail_time = random.randint(60, 180)
            user.jail_until = time.time() + jail_time
            user.bribe_available_at = time.time() + int(jail_time * BRIBE_WAIT_RATIO)
