...
INTERNATIONAL_CITIES = ["Dublin", "Glasgow", "Paris", "Amsterdam", "Havana"]
# Self-smuggling/travel is free. Players can move themselves without ticket costs,
# but carrying contraband can still trigger customs/police risk.
INTERNATIONAL_TRAVEL_COST = 0
...
def travel_cost_to(destination):
    # Self-smuggling is free: only shipment services cost money.
    return 0
...
<div class="main">
    ...
    {% if page == "market" %}
        ...
        <hr><h2>🚂 Self-Smuggling Travel</h2><p>Travelling yourself is <b class="good">free</b>. If you carry warehouse stock, police/customs can still catch you.</p>
        <form action="/travel_action" method="post">
            <select class="input" name="destination">
            {% for city in cities %}<option value="{{ city }}" {% if city == user.location %}disabled{% endif %}>{{ city }} - Free{% if is_international_city(city) %} 🌍{% endif %}</option>{% endfor %}
            </select>
...
def travel_action():
    ...
    db.session.commit()
    return redirect(url_for("market", msg=f"You travelled to {destination} for free."))
