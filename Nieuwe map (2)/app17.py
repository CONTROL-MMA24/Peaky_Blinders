...
<div class="app-shell">
    ...
    {% if page == "assets" %}
        ...
        <hr>
        <h2>🔫 Weapon Factories in {{ user.location }}</h2>
        <p class="muted">Bullets and weapons bought in the Weapons page come from local weapon factories. If the local factories have no stock, players cannot buy there until factories produce more.</p>
        <table>
            <tr><th>Factory</th><th>Owner</th><th>Bullet Stock</th><th>Weapon Stock</th><th>Price</th><th>Action</th></tr>
            ...
            <tr>
                ...
                <td class="gold">{{ moneyfmt(row.bullet_stock) }}</td>
                <td class="gold">{{ moneyfmt(row.weapon_stock) }}</td>
                <td>${{ moneyfmt(row.price) }}</td>
                <td>
                    {% if row.is_state %}
                    <form action="/asset_action" method="post">
                        <input type="hidden" name="slot_number" value="{{ row.factory.slot_number }}">
                        <button class="btn" name="action" value="buy_weapon_factory">Buy Factory</button>
                    </form>
                    {% elif row.factory.owner_id == user.id %}
                        <span class="good">Owned by you</span>
                    {% else %}
                        <span class="red">Owned</span>
                    {% endif %}
                </td>
            </tr>
            {% endfor %}
