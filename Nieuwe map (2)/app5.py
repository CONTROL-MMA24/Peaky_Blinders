...
<div class="app-shell">
    ...
    {% if page == "crime" %}
        ...
        <p class="muted">Vehicle theft is split into street theft and player theft. Street theft has low rewards and very low odds on luxury vehicles. Player theft is stronger, but victims with bodyguards, safehouses and lookouts are harder to rob.</p>
        <div class="grid">
            <div class="card">
        ...
        <div class="grid">
            <div class="card">
                <h3>Street Vehicle Theft</h3>
                <p>Steal from streets and parking garages. Most attempts find parts or nothing. Luxury vehicles are very rare.</p>
                <p class="muted">Low reward · low luxury chance · no player target.</p>
                <form action="/crime_action" method="post">
                    <button class="btn" name="crime" value="street_vehicle_theft">Search Parking Garages</button>
                </form>
            </div>
            <div class="card">
                <h3>Steal From Player</h3>
                <p>The system searches local garages in <b class="gold">{{ user.location }}</b> and automatically picks a random active player with a stealable vehicle.</p>
                <p class="muted">No username needed. If successful, the stolen vehicle moves into your local garage. Bodyguards, safehouses and lookouts reduce your chance.</p>
                <form action="/crime_action" method="post">
                    <button class="btn" name="crime" value="player_vehicle_theft">Search Local Garages</button>
                </form>
            </div>
        </div>
    {% endif %}
    ...
    {% if page == "garage" %}
        ...
        <div class="garage-command">
            <div>
                <h1>GARAGE</h1>
                <p>Your vehicles, from classic rides to private jets.</p>
            </div>
            <div class="garage-summary-panel">
                <div><small>TOTAL VEHICLES</small><b>{{ total_vehicles(user) }}</b></div>
                <div><small>TOTAL VALUE</small><b>${{ moneyfmt(empire_value(user)) }}</b></div>
                <div><small>GARAGE SPACE</small><b>{{ local_owned_count }} / 20</b></div>
        ...
        <div id="vehicle-showroom">
        {% for group in vehicle_categories %}
            <div class="vehicle-section">
                <h2 class="vehicle-section-title">{{ group.name }}</h2>
                <div class="vehicle-grid">
                    {% for vehicle in group.vehicles %}
                    <div class="vehicle-card aaa-vehicle-card">
                        <div class="vehicle-image" style="--vehicle-tone: {{ vehicle.tone }}; {% if vehicle.image %}background-image: linear-gradient(rgba(0,0,0,.08),rgba(0,0,0,.72)), url('{{ vehicle.image }}');{% endif %}">
                            <div class="vehicle-silhouette">
                        ...
                        <div class="vehicle-body">
                            <div class="vehicle-specs">
                                <span>🏎 {{ 90 + vehicle.bonus * 3 }} km/h</span>
                                <span>⛽ {% if vehicle.key == 'private_jet' %}Jet Fuel{% else %}{{ 18 - (vehicle.bonus // 8) }} L / 100km{% endif %}</span>
                            </div>
                            <div class="vehicle-meta compact">
                            ...
                            <form action="/garage_action" method="post">
                                ...
                                <button class="btn drive-btn">{% if vehicle.key == 'private_jet' %}✈ FLY{% else %}☸ DRIVE{% endif %}</button>
                            </form>
                        </div>
                    </div>
                    {% endfor %}
