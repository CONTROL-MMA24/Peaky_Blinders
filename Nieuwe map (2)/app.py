...
<div class="app-shell">
    <aside class="sidebar">
        ...
        <div class="sidebar-player">
            👤 {{ user.username }}<br>
            📍 <span class="city">{% if is_user_traveling(user) %}TRAVELLING{% else %}{{ user.location }}{% endif %}</span><br>
            🏆 {{ user.rank }}
        </div>
        <div class="nav">
            {% set unread = unread_message_count(user) %}

            <div class="menu-section">
                <div class="menu-header">🏠 Overview</div>
                <a href="/dashboard" class="{% if page == 'dashboard' %}active{% endif %}">🏠 Dashboard</a>
                <a href="/messages" class="{% if page == 'messages' %}active{% endif %}">📬 Messages{% if unread > 0 %}<span class="badge">{{ unread }}</span>{% endif %}</a>
            ...
            <div class="menu-section">
                ...
                <a href="/traveling" class="{% if page == 'traveling' %}active{% endif %}">✈️ Traveling</a>
            ...
            <div class="menu-section">
                ...
                <a href="/protection" class="{% if page == 'protection' %}active{% endif %}">🛡️ Protection</a>
            ...
            <div class="menu-section">
                ...
                <a href="/territories" class="{% if page == 'territories' %}active{% endif %}">⚔️ Territories</a>
            </div>
    ...
    <main class="content-wrap">
        <div class="topbar"><div><div class="hero-title">{{ page_title }}</div><div class="muted">{{ user.username }} · {{ user.rank }}</div></div><div class="player-strip">{% if is_user_traveling(user) %}<span class="pill">✈️ {{ user.travel_origin or user.location }} ➜ {{ user.travel_destination }} · {{ format_duration(travel_remaining(user)) }}</span>{% else %}<span class="pill page-city">📍 {{ user.location }}</span>{% endif %}<span class="pill">👪 {% if user.family %}{{ user.family.name }}{% else %}Solo{% endif %}</span><span class="pill">⚡ {{ power_score(user) }}</span><span class="pill">🏴 {{ family_territory_count(user.family) if user.family else 0 }} territories</span><span class="pill">🎫 {% if user.casino_license %}Licensed{% else %}No License{% endif %}</span></div></div>
