...
def create_message(user_id, category, title, body, commit=True):
    if not user_id:
        return None
    message = Message(user_id=user_id, category=category or "system", title=title, body=body, is_read=False, created_at=time.time())
...
.top-icons:after{content:'🎩⌄';width:56px;height:48px;border:1px solid rgba(255,204,116,.34);border-radius:50%;display:grid;place-items:center;color:var(--aaa-gold2);background:radial-gradient(circle,#182b33,#01080b 72%);font-size:24px;}
...
.language-picker{display:flex;align-items:center;gap:8px;justify-content:flex-end;min-width:150px;padding-left:10px}
.language-picker label{font-size:10px;font-weight:900;letter-spacing:1.2px;text-transform:uppercase;color:var(--aaa-muted,#a99b88)}
.language-select{appearance:none;background:linear-gradient(135deg,rgba(5,14,18,.95),rgba(28,18,7,.92));border:1px solid rgba(217,154,43,.42);color:#ffe7b0;border-radius:999px;padding:9px 30px 9px 12px;font-size:12px;font-weight:800;max-width:148px;cursor:pointer;box-shadow:0 10px 28px rgba(0,0,0,.22)}
.language-picker:after{content:"▾";margin-left:-28px;color:#d99a2b;pointer-events:none;font-size:11px}
.language-select option{background:#071015;color:#ffe7b0}

/* === Compact language selector / topbar right alignment fix === */
.aaa-topbar{grid-template-columns:repeat(6,minmax(112px,1fr)) 250px!important;}
.compact-top-icons{display:flex!important;align-items:center!important;justify-content:flex-end!important;gap:8px!important;min-width:250px!important;overflow:visible!important;}
.compact-top-icons:after{content:"🎩"!important;width:42px!important;height:42px!important;flex:0 0 42px!important;border:1px solid var(--aaa-line)!important;border-radius:50%!important;display:grid!important;place-items:center!important;background:radial-gradient(circle,#192a31,#02080b 72%)!important;color:var(--aaa-gold2)!important;font-size:21px!important;}
.topbar-pill{height:36px!important;min-width:52px!important;display:flex!important;align-items:center!important;justify-content:center!important;gap:5px!important;padding:0 10px!important;border-radius:10px!important;border:1px solid rgba(212,175,55,.25)!important;background:rgba(255,255,255,.035)!important;color:#ffe7b0!important;font-size:12px!important;font-weight:800!important;line-height:1!important;white-space:nowrap!important;position:relative!important;box-shadow:0 8px 20px rgba(0,0,0,.18)!important;}
.notification-pill i{position:static!important;right:auto!important;top:auto!important;min-width:auto!important;height:auto!important;border:0!important;background:transparent!important;color:#fff!important;border-radius:0!important;font-style:normal!important;font-size:12px!important;display:inline!important;place-items:normal!important;}
.language-dropdown{cursor:pointer!important;min-width:64px!important;}
.language-menu{display:none!important;position:absolute!important;top:42px!important;right:0!important;min-width:118px!important;background:#071015!important;border:1px solid rgba(212,175,55,.35)!important;border-radius:10px!important;overflow:hidden!important;z-index:99999!important;box-shadow:0 18px 45px rgba(0,0,0,.55)!important;}
.language-dropdown:hover .language-menu{display:block!important;}
.language-menu form{margin:0!important;}
.language-menu button{width:100%!important;border:0!important;background:transparent!important;color:#ffe7b0!important;text-align:left!important;padding:10px 12px!important;font-size:12px!important;font-weight:800!important;cursor:pointer!important;font-family:inherit!important;}
.language-menu button:hover{background:rgba(212,175,55,.15)!important;color:#fff!important;}
@media(max-width:1200px){.aaa-topbar{grid-template-columns:repeat(3,1fr)!important}.compact-top-icons{justify-content:flex-start!important;min-width:0!important;grid-column:1/-1!important}.compact-top-icons:after{display:none!important}}

@media(max-width:900px){.language-picker{justify-content:flex-start;padding:8px 0}.language-select{max-width:100%}}
...
.top-icons:after{content:"🎩"!important;width:48px!important;height:48px!important;border:1px solid var(--aaa-line)!important;border-radius:50%!important;display:grid!important;place-items:center!important;background:radial-gradient(circle,#192a31,#02080b 72%)!important;color:var(--aaa-gold2)!important;}
...
.s3-hero-side{position:relative;z-index:1;background:linear-gradient(145deg,rgba(0,0,0,.35),rgba(217,154,43,.08));border:1px solid rgba(217,154,43,.34);border-radius:22px;padding:18px;display:grid;gap:12px;}
.s3-status-line{display:flex;justify-content:space-between;gap:12px;border-bottom:1px solid rgba(217,154,43,.16);padding-bottom:10px;color:#bba989;}
...
<div class="app-shell">
    <aside class="sidebar">
        ...
        <div class="sidebar-player aaa-player">
            <div class="avatar-frame"><div class="avatar-face">♠</div></div>
    ...
    <main class="content-wrap">
        <div class="topbar aaa-topbar">
            ...
            <div class="top-icons compact-top-icons">
                <span class="topbar-pill notification-pill">✉ <i>{{ unread_message_count(user) }}</i></span>
                <span class="topbar-pill notification-pill">🔔 <i>{{ family_territory_count(user.family) if user.family else 0 }}</i></span>
                <div class="topbar-pill language-dropdown">
                    ...
                    <div class="language-menu">
                        {% for code, info in languages.items() %}
                            <form action="/set_language" method="post">
                                ...
                                <button type="submit">{{ info.flag }} {{ code|upper }}</button>
                            </form>
                        {% endfor %}
                    </div>
                </div>
    ...
    {% if page == "messages" %}
        ...
        <p class="muted">All empire alerts in one place: cargo, travel, crime, security, casino, family and territory events.</p>
        <div class="category-tabs">
            <a href="/messages">All</a><a href="/messages?category=cargo">📦 Cargo</a><a href="/messages?category=travel">✈️ Travel</a><a href="/messages?category=crime">🚨 Crime</a><a href="/messages?category=security">🛡️ Security</a><a href="/messages?category=casino">🎰 Casino</a><a href="/messages?category=territory">⚔️ Territory</a><a href="/messages?category=system">⚙️ System</a>
        </div>
        {% if messages %}
        <div class="message-list">
            ...
            <div class="message-card {% if not message.is_read %}unread{% endif %}">
                ...
                <div>
                    ...
                    <div class="message-meta">{{ message.category|title }} · {{ message_age(message) }}{% if not message.is_read %} · <span class="gold">Unread</span>{% endif %}</div>
                    <div class="message-body">{{ message.body }}</div>
                </div>
                <div>
                    <form action="/message_action" method="post">
                        <input type="hidden" name="message_id" value="{{ message.id }}">
                        {% if not message.is_read %}<button class="btn" name="action" value="read">Mark Read</button>{% endif %}
        ...
        {% else %}
            <div class="card"><h3>No messages</h3><p class="muted">Your inbox is empty.</p></div>
        {% endif %}
    {% endif %}
    ...
    {% if page == "crime" %}
        <div class="s4-ops-wrap">
            ...
            <div class="s4-grid">
                ...
                <article class="s4-operation-card {% if crime_wait > 0 %}s4-disabled{% endif %}">
                    <div class="s4-card-top">
                        <div style="display:flex;gap:12px;align-items:start"><div class="s4-icon">{{ op.icon }}</div><div class="s4-title"><h3>{{ op.title }}</h3><p>{{ op.desc }}</p></div></div>
            ...
            <div class="s4-grid two">
                <article class="s4-operation-card {% if crime_wait > 0 %}s4-disabled{% endif %}">
                    <div class="s4-card-top"><div style="display:flex;gap:12px;align-items:start"><div class="s4-icon">🚗</div><div class="s4-title"><h3>Street Vehicle Theft</h3><p>Search streets and parking garages. Most attempts find parts or nothing; luxury vehicles are rare.</p></div></div><span class="s4-risk risk-medium">Medium Risk</span></div>
                ...
                <article class="s4-operation-card {% if crime_wait > 0 %}s4-disabled{% endif %}">
                    <div class="s4-card-top"><div style="display:flex;gap:12px;align-items:start"><div class="s4-icon">🎯</div><div class="s4-title"><h3>Steal From Player</h3><p>Automatically targets a local active player with stealable vehicles in {{ user.location }}.</p></div></div><span class="s4-risk risk-high">High Risk</span></div>
                        ...
                        <div class="vehicle-card aaa-vehicle-card sprint2-vehicle-card {% if owned_here > 0 %}is-owned{% endif %}">
                            ...
                            <div class="vehicle-body sprint2-vehicle-body">
                                ...
                                <form action="/garage_action" method="post">
                                    ...
                                    <button class="btn drive-btn sprint2-buy-btn">PURCHASE</button>
                                </form>
                            </div>
                        </div>
                        {% endfor %}
        ...
        <div class="s4-ops-wrap">
            ...
            <div class="s4-grid">
                ...
                <article class="s4-operation-card {% if heist_wait > 0 or not plan.ready %}s4-disabled{% endif %}">
                    <div class="s4-card-top">
                        <div style="display:flex;gap:12px;align-items:start"><div class="s4-icon">{% if plan.key == 'bookmaker' %}📒{% elif plan.key == 'train' %}🚂{% else %}🚔{% endif %}</div><div class="s4-title"><h3>{{ plan.name }}</h3><p>{% if plan.key == 'bookmaker' %}Shake down bookmakers and collect street debt.{% elif plan.key == 'train' %}Hit a payroll convoy before it reaches the depot.{% else %}Raid a police armory for a high-pressure score.{% endif %}</p></div></div>
...
PAGE_TITLE_KEYS = {"dashboard":"dashboard","messages":"messages","crime":"crimes","bank":"bank","market":"smuggling","traveling":"traveling","cargo":"cargo","warehouse":"warehouse","assets":"businesses","properties":"properties","garage":"garage","casino":"casinos","licenses":"licenses","influence":"influence","family":"family","territories":"territories","bullets":"weapons","protection":"protection","heists":"heists","jail":"jail","ranking":"rankings"}
PAGE_ICONS = {"dashboard":"🏠","messages":"📬","crime":"⚡","bank":"🏦","market":"🍾","traveling":"✈️","cargo":"📦","warehouse":"🏚","assets":"🏭","properties":"🏛","garage":"🚗","casino":"🃏","licenses":"🎫","influence":"🏛","family":"👪","territories":"⚔️","bullets":"🔫","protection":"🛡️","heists":"💼","jail":"🚔","ranking":"🏆"}
...
def messages():
    ...
    return render_page(user, "messages", messages=message_rows(user, category), msg=request.args.get("msg"))
...
def message_action():
    ...
    if action == "read":
        ...
        db.session.commit()
        return redirect(url_for("messages", msg="Message marked as read."))
    if action == "delete":
        db.session.delete(message)
        db.session.commit()
        return redirect(url_for("messages", msg="Message deleted."))
